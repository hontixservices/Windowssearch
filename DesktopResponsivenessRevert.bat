@echo off
    ...
    wmic process where name="csrss.exe"call sertpriority "realtime"
...
Reg.exe add "HKLM\SYSTEM\CurrentControlSet\Services\GpuEnergyDr" /v "Start" /t REG_DWORD /d "4" /f
    ...
    reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\Dwm.exe\PerfOptions" /v "CpuPriorityClass" /t REG_DWORD /d 3 /f
